...
- **`Failed to fetch` ya no se atribuye siempre a la conexión**: ese error también aparece cuando el `500` del servidor de correo llega sin cabeceras CORS y el navegador no deja leer la respuesta. Ahora el mensaje distingue según `navigator.onLine`: sin conexión culpa a internet; en línea señala el servidor de correo y ofrece WhatsApp.
