...
async function processItem(item: QueueItem) {
  ...
  switch (type) {
    case 'SALE': {
      ...
      // Mantenemos tu lógica original para ventas
...
// =========================================================
// FUNCIÓN DE CARGA INICIAL COMPLETA (OPCIÓN A - BLOQUEANTE)
// =========================================================
...
export async function syncBusinessProfile(businessId: string) {
  ...
  try {
    console.log('🚀 Iniciando descarga TOTAL de datos (Productos, Clientes, Staff)...');
    
    // Ejecutamos TODAS las peticiones en paralelo.
    // Esto descargará TODO antes de dejar entrar al usuario.
    const [
        businessResult, 
        staffResult, 
        registersResult, 
    ...
    if (staffResult.data && staffResult.data.length > 0) {
      await db.staff.clear(); // Limpiamos para evitar duplicados viejos
      await db.staff.bulkPut(staffResult.data);
      console.log(`✅ ${staffResult.data.length} Empleados.`);
    }
    ...
    // 4. Productos (¡La parte crítica que faltaba!)
    if (productsResult.data && productsResult.data.length > 0) {
        const cleanProducts = productsResult.data.map(p => ({
            ...p,
            sync_status: 'synced' // Marcamos como sincronizados
        }));
        // Usamos bulkPut para insertar rápido miles de productos
        await db.products.bulkPut(cleanProducts);
        console.log(`✅ ${productsResult.data.length} Productos descargados.`);
    } else if (productsResult.data?.length === 0) {
        console.log('ℹ️ El negocio no tiene productos en la nube aún.');
    }
    ...
    if (customersResult.data && customersResult.data.length > 0) {
        ...
        console.log(`✅ ${customersResult.data.length} Clientes descargados.`);
    }

    console.log('🏁 Sincronización TOTAL completada.');

  } catch (error) {
    console.error('⚠️ Error en sincronización masiva:', error);
  }
}